...
export class RecoverComponent implements OnInit {
      ...
      .subscribe(
        response => {
          if (response.status >= 200 && response.status < 400) {
            ...
            this.AlertIfNeeded(bitcoinErrorMessage);
        },
        error => {
  ...
  private AlertIfNeeded(bitcoinErrorMessage: string) {
        if(bitcoinErrorMessage !== "") {
          let errorMessage = "<strong>Bitcoin wallet recovery:</strong><br>" + bitcoinErrorMessage + "<br><br>";
